...
class Twig_Tests_ErrorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testTwigExceptionAddsFileAndLineWhenMissingWithInheritanceOnDisk()
    {
        ...
        $template = $twig->loadTemplate('index-template.html');
        try {
            $template->render(array());

            $this->fail();
        } catch (Twig_Error_Runtime $e) {
            $this->assertEquals('Variable "foo" does not exist in "index-template.html" at line 3', $e->getMessage());
            $this->assertEquals(3, $e->getTemplateLine());
            $this->assertEquals('index-template.html', $e->getTemplateFile());
        }
        ...
        try {
            ...
            $this->assertEquals('An exception has been thrown during the rendering of a template ("Runtime error...") in "index-template.html" at line 3.', $e->getMessage());
            $this->assertEquals(3, $e->getTemplateLine());
            $this->assertEquals('index-template.html', $e->getTemplateFile());
        }
    }
